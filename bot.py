...
# --- 2. Песня через OpenRouter (текст) ---
async def generate_song_lyrics(topic: str) -> str:
    ...
    try:
        ...
        data = {
            ...
            "messages": [{"role": "user", "content": f"Напиши текст песни на русском языке на тему: {topic}. Только текст песни, с куплетами и припевом."}],
            "max_tokens": 800
        }
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=data, headers=headers) as resp:
                if resp.status != 200:
                    ...
                    return f"❌ Ошибка API (статус {resp.status}): {text[:100]}"
...
# --- 3. Анализ фото (Groq) ---
async def analyze_photo_with_groq(photo_url: str) -> str:
    try:
        response = groq_client.chat.completions.create(
            model="llama-3.2-11b-vision-preview",
            messages=[
                {"role": "user", "content": [
                    {"type": "text", "text": "Опиши подробно, что ты видишь на этом фото, на русском языке."},
        ...
        return f"❌ Анализ через Groq временно недоступен. Ошибка: {str(e)[:100]}\n\nПодсказка: попробуй отправить фото с более простым содержанием."

# --- 4. Поиск объектов (через Groq) ---
async def describe_objects_with_groq(photo_url: str) -> str:
    try:
        response = groq_client.chat.completions.create(
            model="llama-3.2-11b-vision-preview",
            messages=[
                {"role": "user", "content": [
                    {"type": "text", "text": "Перечисли все объекты, которые ты видишь на этом фото, на русском языке. Напиши в виде списка: - объект1, - объект2."},
                    {"type": "image_url", "image_url": {"url": photo_url}}
                ]}
            ],
            max_tokens=300
        ...
        return f"❌ Поиск объектов временно недоступен. Ошибка: {str(e)[:100]}"

# --- 5. Распознавание текста (через Groq) ---
async def extract_text_with_groq(photo_url: str) -> str:
    try:
        response = groq_client.chat.completions.create(
            model="llama-3.2-11b-vision-preview",
            messages=[
                {"role": "user", "content": [
                    {"type": "text", "text": "Найди и вытащи ВЕСЬ текст с этого фото. Если текста нет, напиши 'Текст не найден'."},
                    {"type": "image_url", "image_url": {"url": photo_url}}
                ]}
            ],
            max_tokens=800
        ...
        return f"❌ Распознавание текста временно недоступно. Ошибка: {str(e)[:100]}"

# --- 6. Редактирование фото ---
...
async def analyze_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔍 Отправь ФОТО для анализа.")
    context.user_data['awaiting_analyze'] = True

async def detect_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👁️ Отправь ФОТО для поиска объектов.")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('awaiting_analyze'):
        ...
        result = await analyze_photo_with_groq(photo_url)
        await update.message.reply_text(f"🔎 *Результат:*\n{result}", parse_mode="Markdown")
        del context.user_data['awaiting_analyze']
    elif context.user_data.get('awaiting_detect'):
        await update.message.reply_text("👁️ Ищу объекты...")
        result = await describe_objects_with_groq(photo_url)
        ...
        result = await extract_text_with_groq(photo_url)
        await update.message.reply_text(result, parse_mode="Markdown")
        ...
        await update.message.reply_text("📸 Что сделать с фото? Нажми: 🔍 Анализ, 👁️ Объекты, 📖 Текст, ✏️ Редакт", reply_markup=get_main_keyboard())
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif text in get_edit_keyboard().keyboard[0]:
...
def main():
    ...
    print("🚀 Бот AI 3.0 запущен! Анализ фото работает через Groq Vision.")
    app.run_polling()
